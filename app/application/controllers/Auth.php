...
class Auth extends CI_Controller
{
            ...
            if ($query->num_rows() == 1) {
                ...
                if (password_verify($password, $row->password)) {
                    if ($rembemr == 'on') {
                        ...
                        setcookie('velixs_token', hash('ripemd160', $row->password), time() + (10 * 365 * 24 * 60 * 60), '/');
                    }
                ...
                echo 'Username atau email tidak terdaftar.';
            }
        } else {
            frontEnd_view('auth_login');
            if (is_login(false)) {
                redirect(base_url(''));
            }
        }
        ...
        if ($this->input->post()) {
            ...
            if ($this->db->get_where('users', ['email' => $email])->num_rows() == 0) {
                if ($this->db->get_where('users', ['username' => $username])->num_rows() == 0) {
                    ...
                    $this->session->set_flashdata('success', 'Berhasil Daftar, Silahkan melakukan login.');
                    ...
                    $this->session->set_flashdata('error', 'Username sudah terdaftar di akun lain.');
                ...
                $this->session->set_flashdata('val_password', $password);
                $this->session->set_flashdata('val_email', $email);
                $this->session->set_flashdata('error', 'Email sudah terdaftar di akun lain.');
                redirect(base_url("auth/register"));
            }
        } else {
            frontEnd_view('auth_register');
            if (is_login(false)) {
                redirect(base_url(''));
            }
        }
    ...
    public function logout()
    {
        ...
        unset($_COOKIE['velixs_token']);
        unset($_COOKIE['velixs_id']);
        setcookie('velixs_token', null, -1, '/');
        setcookie('velixs_id', null, -1, '/');
        ...
        if ($query->num_rows() == 1) {
            ...
            if ($row->tipe == 'verify') {
                ...
                if ($this->input->post()) {
                    ...
                    ajax_loadview('auth_update_password');
                }
            }
        } else {
            ajax_loadview('auth_expired');
        }
    }
        ...
        if ($this->input->post()) {
            ...
            if ($tipe == 'cari') {
                ...
                if ($cekusers->num_rows() != 0) {
                    $row         = $cekusers->row();
                    $data['row'] = $row;
                    frontEnd_view('auth_forgot', $data);
                } else {
                ...
                if ($this->session->userdata('session_forgot')) {
                    if ($this->session->userdata('session_forgot') < date('Y-m-d H:i')) {
                        ...
                        $this->session->set_flashdata('error', 'Mengirim Email Forgot Cooldown 5 menit.');
                        redirect(base_url("auth/forgot"));
                    }
                } else {
                    $this->kirim_forgot($us);
            ...
            frontEnd_view('auth_forgot');
        }
    }

    public function kirim_verify($row)
    {
        $minutes_to_add = 5;
        ...
        $this->session->set_flashdata('success', 'Berhasil kirim link verify di email ' . $row->email . '.');
        redirect(base_url("verify"));
    }

    public function kirim_forgot($row)
    {
        $minutes_to_add = 5;
        $time           = new DateTime(date('Y-m-d H:i'));
        $time->add(new DateInterval('PT' . $minutes_to_add . 'M'));
        ...
        $this->sendEmail('Forgot CTF', $msg, $row->email);
        $this->session->set_flashdata('success', 'Berhasil kirim link lupa password cek email kamu di ' . $row->email . '.');
        redirect(base_url("auth/forgot"));
    }

    public function sendEmail($title, $msg, $penerima)
    {
